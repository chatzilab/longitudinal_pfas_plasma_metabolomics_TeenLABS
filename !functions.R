...
fit_lmm_random_intercept_covars <- function(x, y, key, visit, covariates_df){
  ...
  df <- as.data.frame(cbind(df, covariates_df))
  df$y <- as.numeric(df$y)
  model <- as.formula(paste("y ~ x*visit", "+", paste(colnames(covariates_df)[! colnames(covariates_df) == "key"], collapse = " + ")))
  reduced_model <- as.formula(paste("y ~ x + visit", "+", paste(colnames(covariates_df)[! colnames(covariates_df) == "key"], collapse = " + ")))
  fit <- lme(model,
             random = ~1|key,
             data = df, na.action=na.omit)
  coef_table <- summary(fit)$tTable
  ...
  if(length(fit_reduced) == 1)
  {
    ...
    overall_sig <- anova(update(fit, . ~ ., method = "ML"),
                         update(fit_reduced, . ~ ., method = "ML"))
    output <- cbind(output,
                    c(overall_sig$`p-value`[2], rep(NA, time = 1)))
    output <- cbind(output,
                    c(overall_sig$L.Ratio[2], rep(NA, time = 1)))
  }
...
get_annotation <- function(feature_vec, annotation_df)
{
  ...
  feature_vec <- feature_vec[!is.na(names(feature_vec)) & names(feature_vec) != ""]
  
  return(feature_vec)
}

# get significant count
get_sig_by_chemical <- function(df, var, threshold){
  # Select only the interaction term rows (every second row)
  interaction_rows <- df[seq(2, nrow(df), by = 2),]
...
get_sig_features_by_chemical <- function(df, var, threshold) {
  ...
  feature_list_full <- setNames(vector("list", length(chemical_variables)), chemical_variables)
...
fit_lmm_random_intercept_covars_CI <- function(x, y, key, visit, covariates_df){
  df <- as.data.frame(cbind(key, x, y, visit))
  names(df) <- c("key", "x", "y", "visit")
  df <- as.data.frame(cbind(df, covariates_df))
  df$y <- as.numeric(df$y)
  ...
  colnames(output) <- c("coef", "se", "statistic", "pvalue", "conf_low", "conf_high", "pvalue_interaction", "L_ratio")
...
fit_lmm_random_intercept_covars_JT <- function(x, y, key, visit, covariates_df){
  df <- as.data.frame(cbind(key, x, y, visit))
  names(df) <- c("key", "x", "y", "visit")
  df <- as.data.frame(cbind(df, covariates_df))
  df$y <- as.numeric(df$y)
  model <- as.formula(paste("y ~ x*visit", "+", paste(colnames(covariates_df)[! colnames(covariates_df) == "key"], collapse = " + ")))
  reduced_model <- as.formula(paste("y ~ visit", "+", paste(colnames(covariates_df)[! colnames(covariates_df) == "key"], collapse = " + ")))
  fit <- lme(model,
             random = ~1|key,
             data = df, na.action=na.omit)
  coef_table <- summary(fit)$tTable
...
# Linear mixed-effect models without spline of time + joint test
# with confidence interval
fit_lmm_random_intercept_covars_JT_CI <- function(x, y, key, visit, covariates_df){
  df <- as.data.frame(cbind(key, x, y, visit))
  names(df) <- c("key", "x", "y", "visit")
  df <- as.data.frame(cbind(df, covariates_df))
  df$y <- as.numeric(df$y)
